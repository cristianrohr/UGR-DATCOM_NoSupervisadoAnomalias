...
# As? que ninguno de los dos m?todos basados en la distancia de Mahalanobis funciona bien
...
# en un gr?fico 2 dimensional, aunque la aproximaci?n es muy buena ya que la suma de las varianzas
...
plot(lof.scores)
numero.de.outliers = 4
indices.de.lof.outliers.ordenados <- sort(lof.scores, index.return = TRUE, decreasing = TRUE)$ix
...
MiBiPlot_Multivariate_Outliers(mis.datos.numericos.normalizados, is.lof.outlier, "Biplot outliers")
...
indices.de.outliers.multivariantes.LOF.pero.no.1variantes <- setdiff(indices.de.lof.top.outliers,
                                                                     vector.claves.outliers.IQR.en.alguna.columna
                                                                     )
